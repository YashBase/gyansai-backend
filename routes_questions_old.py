"""Question bank routes — Complete CRUD, filters, image upload, pagination."""
import base64
import json
import logging
import re
...
def _local_upload_path(filename: str) -> tuple[str, Path]:
...
async def get_prev_question(
    ...
    """Get previous question (by created_at) given filters."""
...
async def get_next_question(
    ...
    """Get next question (by created_at) given filters."""
    current = await db.questions.find_one({"id": qid}, {"_id": 0})
    if not current:
        raise HTTPException(status_code=404, detail="Question not found")
    ...
    try:
        ...
        if inserted_ids:
            try:
                ...
                logger.exception("Failed to rollback inserted questions")
